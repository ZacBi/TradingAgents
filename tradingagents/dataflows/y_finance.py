...
def get_YFin_data_online(
    ...
    try:
        ...
        price_serializable = data.head(200).to_dict(orient="records") if not data.empty else None
        try_record_raw_market_data(
            ticker=symbol,
            trade_date=end_date,
            price_data=price_serializable,
            source="yfinance",
        )
    except Exception:
        pass
...
def _get_stock_stats_bulk(
    ...
    if not online:
        ...
        try:
            ...
            raise RuntimeError(
                "Data fetch failed: Yahoo Finance data not cached yet!"
            ) from err
    else:
