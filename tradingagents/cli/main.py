...
def _cli_callback() -> None:
    """Run before any command; load .env so config/API keys are available."""
    load_dotenv()
...
def _agent_status_cell(status: str):
    """Return Rich renderable for an agent status (spinner or colored text)."""
...
def _build_progress_table():
    """Build the progress panel table from message_buffer.agent_status."""
    table = Table(
        show_header=True,
    ...
    teams = {t: a for t, a in teams.items() if a}

    for team, agents in teams.items():
        for i, agent in enumerate(agents):
            status = message_buffer.agent_status.get(agent, "pending")
            team_name = team if i == 0 else ""
            table.add_row(team_name, agent, _agent_status_cell(status))
        table.add_row("─" * 20, "─" * 20, "─" * 20, style="dim")
    return table


def _build_messages_table():
    """Build the messages/tools panel table from message_buffer."""
    table = Table(
        show_header=True,
        header_style="bold magenta",
        show_footer=False,
    ...
    for timestamp, tool_name, args in message_buffer.tool_calls:
        all_messages.append(
            (timestamp, "Tool", f"{tool_name}: {format_tool_args(args)}")
        )
...
def _build_footer_stats(stats_handler, start_time):
    """Build footer stats parts and return a single string for the stats row."""
    ...
    parts = [f"Agents: {agents_completed}/{agents_total}"]
    ...
    if stats_handler:
        ...
        if stats["tokens_in"] > 0 or stats["tokens_out"] > 0:
            ...
            parts.append("Tokens: --")
    parts.append(f"Reports: {reports_completed}/{reports_total}")
    if start_time:
...
def update_display(layout, spinner_text=None, stats_handler=None, start_time=None):
    """Refresh the live layout with header, progress, messages, analysis, footer."""
    ...
    report_content = (
        ...
        else "[italic]Waiting for analysis report...[/italic]"
    )
    ...
    stats_table.add_row(_build_footer_stats(stats_handler, start_time))
...
def _analyst_report_items(state) -> list[tuple[str, str, str]]:
    """(filename, display_name, content) for each analyst report present."""
    items = []
    for key, fname, name in [
        ...
        if state.get(key):
            items.append((fname, name, state[key]))
    return items


def _write_section_dir(base_dir: Path, items: list[tuple[str, str, str]]) -> str | None:
    """Write files to base_dir; return markdown section content or None."""
    if not items:
        return None
    base_dir.mkdir(exist_ok=True)
    for fname, _display_name, text in items:
        (base_dir / fname).write_text(text)
    return "\n\n".join(f"### {n}\n{t}" for _f, n, t in items)


def _research_report_items(state) -> list[tuple[str, str, str]]:
    ...
    for key, fname, name in [
        ...
        if debate.get(key):
            items.append((fname, name, debate[key]))
    return items


def _risk_report_items(state) -> list[tuple[str, str, str]]:
    ...
    for key, fname, name in [
        ...
        if risk.get(key):
            items.append((fname, name, risk[key]))
    return items
...
def save_report_to_disk(final_state, ticker: str, save_path: Path):
    ...
    analyst_items = _analyst_report_items(final_state)
    if analyst_items:
        content = _write_section_dir(save_path / "1_analysts", analyst_items)
        if content:
            sections.append(f"## I. Analyst Team Reports\n\n{content}")

    research_items = _research_report_items(final_state)
    if research_items:
        research_dir = save_path / "2_research"
        research_dir.mkdir(exist_ok=True)
        for fname, _name, text in research_items:
            (research_dir / fname).write_text(text)
        content = "\n\n".join(f"### {n}\n{t}" for _f, n, t in research_items)
    ...
    risk_items = _risk_report_items(final_state)
    if risk_items:
        risk_dir = save_path / "4_risk"
        risk_dir.mkdir(exist_ok=True)
        for fname, _name, text in risk_items:
            (risk_dir / fname).write_text(text)
        content = "\n\n".join(f"### {n}\n{t}" for _f, n, t in risk_items)
...
def _display_report_section(section_title: str, items: list[tuple[str, str]], style: str) -> None:
    """Print a report section with panels for each (title, content)."""
    if not items:
        return
    console.print(Panel(f"[bold]{section_title}[/bold]", border_style=style))
...
def _analyst_display_items(state) -> list[tuple[str, str]]:
    """(display_name, content) for analyst reports."""
    out = []
    for key, name in [
...
def _research_display_items(state) -> list[tuple[str, str]]:
    """(display_name, content) for research debate."""
    debate = state.get("investment_debate_state") or {}
    out = []
    for key, name in [
        ...
        if debate.get(key):
            out.append((name, debate[key]))
    return out


def _risk_display_items(state) -> list[tuple[str, str]]:
    """(display_name, content) for risk debate (excluding portfolio)."""
    risk = state.get("risk_debate_state") or {}
    out = []
    for key, name in [
...
def display_complete_report(final_state):
    ...
    _display_report_section(
        "I. Analyst Team Reports",
        _analyst_display_items(final_state),
        "cyan",
    )
    _display_report_section(
        "II. Research Team Decision",
        _research_display_items(final_state),
        "magenta",
    )
    if final_state.get("trader_investment_plan"):
    ...
    _display_report_section(
        ...
        _risk_display_items(final_state),
        "red",
    )
    risk = final_state.get("risk_debate_state") or {}
    if risk.get("judge_decision"):
...
def _build_analysis_config(selections: dict) -> dict:
    """Build config dict from user selections."""
...
def _prepare_results_dirs(config: dict, selections: dict):
    """Create results/reports dirs and log file; return (results_dir, report_dir, log_file)."""
...
def _install_buffer_logging(message_buffer, log_file: Path, report_dir: Path) -> None:
    """Wrap message_buffer.add_message, add_tool_call, update_report_section to log to files."""
        ...
        def wrapper(section_name, content):
            ...
            if section_name in obj.report_sections and obj.report_sections[section_name]:
                with open(report_dir / f"{section_name}.md", "w") as f:
                    f.write(obj.report_sections[section_name])
        return wrapper
...
def _chunk_handle_messages(message_buffer, chunk: dict) -> None:
    """Add new messages and tool calls from chunk to message_buffer."""
...
def _chunk_handle_investment_debate(message_buffer, debate_state: dict) -> None:
    """Apply investment debate state to buffer and report sections."""
...
def _chunk_handle_risk_debate(message_buffer, risk_state: dict) -> None:
    """Apply risk debate state to buffer and report sections."""
    for key, agent in [
        ("aggressive_history", "Aggressive Analyst"),
        ("conservative_history", "Conservative Analyst"),
        ("neutral_history", "Neutral Analyst"),
    ]:
        hist = (risk_state.get(key) or "").strip()
        if hist:
            if message_buffer.agent_status.get(agent) != "completed":
                message_buffer.update_agent_status(agent, "in_progress")
            message_buffer.update_report_section("final_trade_decision", f"### {agent} Analysis\n{hist}")
...
def _process_stream_chunk(message_buffer, chunk: dict) -> None:
    """Process one stream chunk: messages, analyst status, debate states, report sections."""
    _chunk_handle_messages(message_buffer, chunk)
    update_analyst_statuses(message_buffer, chunk)
    if debate_state := chunk.get("investment_debate_state"):
        _chunk_handle_investment_debate(message_buffer, debate_state)
    ...
    if risk_state := chunk.get("risk_debate_state"):
        _chunk_handle_risk_debate(message_buffer, risk_state)


def run_analysis():
    selections = get_user_selections()
    config = _build_analysis_config(selections)
    ...
    results_dir, report_dir, log_file = _prepare_results_dirs(config, selections)
    _install_buffer_logging(message_buffer, log_file, report_dir)
    ...
    with Live(layout, refresh_per_second=4):
        ...
        for chunk in graph.graph.stream(init_agent_state, **args):
            _process_stream_chunk(message_buffer, chunk)
            update_display(layout, stats_handler=stats_handler, start_time=start_time)
            trace.append(chunk)
        ...
        # Persist decision to DB when enabled (same behavior as graph.propagate())
        if config.get("database_enabled") and hasattr(graph, "persist_decision"):
            graph.persist_decision(final_state, decision)
...
def backtest(
    ...
    from tradingagents.utils.validation import validate_date_range, validate_ticker
